...
class Intervalo(object):
    ...
    def pow_expint(self, otro):
        ...
        if otro.lo <= 0 <= otro.hi:
            ...
            if 1 <= res.lo:
                return Intervalo(math.exp(otro.lo * math.log(res.hi)), math.exp(otro.hi * math.log(res.hi)))
	...
	if (self.lo)*(self.hi) >= 0:
	        ...
	        return Intervalo(min([abs(self.lo),abs(self.hi)]),max([abs(self.lo),abs(self.hi)]))
	else:

		return Intervalo(0.0,max([abs(self.lo),abs(self.hi)]))
